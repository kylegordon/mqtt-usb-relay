...
MQTT_TOPIC = "/raw/" + socket.getfqdn() + "/usb-relay"

APPNAME = "mqtt-usb-relay"
...
def process_message(msg):
    ...
    if RelayID == "1":
        if msg.payload == "1":
            logging.debug("Turning %s on", RelayID)
            ser.write("1")
        if msg.payload == "0":
            logging.debug("Turning %s off", RelayID)
            ser.write("2")
    if RelayID == "2":
        if msg.payload == "1":
            logging.debug("Turning %s on", RelayID)
...
def main_loop():
    ...
    """
    while True:
        # Read the serial input. Relay board continually transmits its state
        msg = ser.readline()

        try:
            logging.debug("Received from serial : %s", msg)
